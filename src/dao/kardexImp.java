    ...
    public boolean save(kardex kardex) {
        ...
        try {
            ...
            if (kardex.getID_KARDEX() == 0) {  //es cero cuando se esta ingresando un registro nuevo: ver inicializac'on del atributo 
                ...
                sql.append("INSERT INTO [dbo].[REGISTRA_KARDEX]([ID_KARDEX],[ID_EMPLEADO],[FECHA_REGISTRO],[CANTIDAD],[TIPO]) VALUES ('").append(kardex.getID_KARDEX());
                sql.append("', '").append(kardex.getID_EMPLEADO());      //crear la cadena de conexion
                sql.append("', '").append(kardex.getFECHA_REGISTRO());      //crear la cadena de conexion
                sql.append("', '").append(kardex.getCANTIDAD());  
                sql.append("', '").append(kardex.getTIPO()).append("')");      //crear la cadena de conexion
                this.conn.execute(sql.toString());      //ejecuta la query
            } else {   //es un registro previamente existente: estamos actualizando
                StringBuilder sql = new StringBuilder();   //para crear la sentencia sql
     
                
                sql.append("UPDATE [dbo].[REGISTRA_KARDEX] SET [ID_EMPLEADO] = ").append(kardex.getID_EMPLEADO());
                ...
                sql.append(", [TIPO] = ").append(kardex.getTIPO()).append(" WHERE I[D_KARDEX] = ").append(kardex.getID_KARDEX());      //crear la cadena de conexion
                this.conn.execute(sql.toString());      //ejecuta la query
            }
